...
- 🎨 **UI/UX Overhaul**: Chuyển đổi cơ chế Unlock từ Options Page về **In-Popup Unlock**.
- 🧹 **Simplification**: Loại bỏ hoàn toàn In-Page Popup (gây che khuất/conflict). Mọi thao tác hiển thị/copy chuyển về Extension Icon Popup.
- ⚡ **Workflow**: Click Icon -> Unlock -> Auto-generate (nếu đang focus cell).
- 🔄 **State Sync**: Đồng bộ trạng thái real-time giữa Popup, Options và Background.
...
AS A user
I WANT TO see the calculated password in the extension popup
SO THAT I can verify it without context switching

Acceptance Criteria:
- [ ] User clicks Extension Icon (toolbar)
- [ ] If locked: Show Unlock Input
- [ ] If unlocked and cell selected: Show Generated Password
- [ ] If unlocked and no cell: Show Dashboard
```

```
US-04: Serve (Copy Password)
AS A user
I WANT TO copy the password to clipboard with one click in the extension popup
SO THAT I can quickly paste it into login forms

Acceptance Criteria:
- [ ] Large "Copy" button in extension popup
- [ ] Visual feedback (Checkmark)
- [ ] Auto-clear clipboard after 30 seconds
- [ ] No intrusive in-page elements (clean UX)
```

```
US-05: Unlock Kitchen (Master Password)
AS A user
I WANT TO unlock the extension directly from the toolbar popup
SO THAT I don't have to open a separate settings page

Acceptance Criteria:
- [ ] Password Input field inside the popup
- [ ] Unlock button
- [ ] Error feedback without closing popup
- [ ] Successful unlock immediately transitions to result/dashboard
...
```
Pattern: <hash><cooking_style><spice_index>[modifiers]
...
### 4.4 Secret Storage Structure (The Pantry)
...
### 6.1 Password Popup (Extension Icon)

Chúng ta đã loại bỏ **In-Page Popup** (Content Script) để tối ưu UX và bảo mật. Giao diện chính nằm ở Toolbar Popup.
...
#### State 2: Unlocked (Dashboard - Idle)
...
│  1. Click a cell in Google Sheets        │
│  2. Click this icon again                │
│                                          │
│  ┌────────────────────────────────────┐  │
│  │ 🔒 Lock Now                        │  │
│  └────────────────────────────────────┘  │
...
#### State 3: Unlocked (Generated Result)
...
├─────────────────────────────────────────────────────────────────┤
│                                                                  │
│  [Setup Section] (Shown if no secrets)                           │
│  ...                                                             │
│                                                                  │
│  [Unlock Section] (Shown if locked)                              │
│  ...                                                             │
│                                                                  │
